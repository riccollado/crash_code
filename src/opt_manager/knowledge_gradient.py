...
def kg_alg(
    ...
    for x in range(1, M):
        ...
        # Obtain restrictions of a, b, c to indexes from A (we have to adjust due to the
        # index disparity of c, a starting from 1 but a, b start from 0) From now on a,
        # b, c, A have the same index set!!!
        a = [a[j - 1] for j in A]
        b = [b[j - 1] for j in A]
        c = [c[j] for j in A]
        M = len(A)
...
def kg_iteration(
    ...
    # Obtain restrictions of a, b, c to indexes from A (we have to adjust due to the
    # index disparity of c, a starting from 1 but a, b start from 0) From now on a, b,
    # c, A have the same index set!!!
    a = [a[j - 1] for j in A]
    b = [b[j - 1] for j in A]
    c = [c[j] for j in A]
    M = len(A)
...
def kg_multi(
    ...
    Returns
    -------
    L : tuple of int and float
        Index of the selected alternative and the value of the objective function.
    """
    M = S.shape[0]
