...
from gen_manager.scenario import dynamic_scenarios
from opt_manager.generator_subproblem import optimize_subproblem
from opt_manager.knowledge_gradient import kg_multi, update_mu_s

kg_leaf_counter = 0
cov_df = pd.DataFrame()


def increment_lc() -> int:
    """Increment leaf counter.

    Returns
    -------
    int
        The updated leaf counter value.
    """
...
def branch_bound_algorithm(
    ...
    ----------
    attributes : dict
        Dictionary containing attributes and parameters needed for the algorithm.
    push_iteration : Callable
        Function to push iteration results to the database.

    Returns
    -------
    return_data : dict
        Dictionary containing the optimal solution and related data.
    """
    global cov_df

    # Step 0: Scenario space initialization
    # We need some extra generated scenarios but we do not want to count them in the
    # total usage
    ...
    # start from a deep tree and there the beliefs would be used. For the moment we'll
    ...
    # Flag to mark when we have just partitioned a record set
    ...
    # Main recursive loop: Iterate over the total number of scenarios.
    while scenario_end_index <= total_scenarios:
        ...
        elapsed_iter_time = time.perf_counter() - start_iter_time
        push_iteration(cov_df, partition_list, attributes, iteration, elapsed_iter_time)

        # Increment iteration and loop
        iteration += 1

    # Get optimal solution (minimal E) among all partitions and get the corresponding
    # variance. If we didn't do bootstrap, the variance values are not so useful.
    return_data = {}
...
def partition_record_set(
    ...
    partition_list : list of dict
        List of subproblems to be partitioned.
    scenario_start_index : int
        Starting index of the scenario range.
    scenario_end_index : int
        Ending index of the scenario range.

    Returns
    -------
    just_partitioned : bool
        States if we have just partitioned (split) the record set in two.
    """
    global cov_df
    ...
    for subproblem in partition_list:
        ...
        if subproblem["recordset"] is True and subproblem["singleton"] is False:
            ...
            # Update cov_df matrix and beliefs for extra subproblems in KG
            if attributes["method_type"] == "KG":
                ...
                # Here we just don't know and set things very simple: mean=0, var=1
                del kg_mu[sub_name]
                kg_mu[sub1_name] = 0.0
                kg_mu[sub2_name] = 0.0
                ...
                cov_df.drop([sub_name], inplace=True)
                cov_df.drop([sub_name], axis=1, inplace=True)
                ...
                # **********cov_df UPDATING FINISHED**********

            # Estimate bounds by taking full sample scenarios at each new leaf. This
            # step also takes care of priming the KG sample statistics.
            scen_est_num = attributes["scen_est_num"]
            estimate_bounds(
                attributes,
                subproblem1,
...
def work_on_subproblems(
    ...
    attributes : dict
        Dictionary containing attributes and parameters needed for the algorithm.
    partition_list : list of dict
        List of subproblems to be worked on.
    scenario_start_index : int
        Starting index of the scenario range.
    scenario_end_index : int
        Ending index of the scenario range.
    ...
    # Assign to each subproblem the scenarios used to update bounds.
    if (
    ...
    for i, subproblem in enumerate(partition_list):
        ...
        # No need to assign scenarios in case of Uniform since every subproblem will get
        ...
        if len(scenarios) != 0:
            ...
            # If no scenarios get allocated then copy the data from last iteration
...
def estimate_bounds(
    ...
    attributes : dict
        Dictionary containing attributes and parameters needed for the algorithm.
    subproblem : dict
        Dictionary representing the subproblem to be solved.
    scenarios : list
        List of scenarios to be considered.

    Returns
    -------
    mean_obj_value : float
        Approximation to expectation obtained from averaging the solution of
        subproblem over every scenario either applying Bootstrap or not.
    """
    global cov_df
    ...
    if attributes["bootstrap"] is True:
        subproblem["Z_E"], subproblem["Z_std"] = bootstrap_mean_std_estimate(
            subproblem, attributes
        )
        # Update mean and std from boostrap
        subproblem["E"] = mean_obj_value = subproblem["Z_E"]
        subproblem["STD"] = std_obj_function_value = subproblem["Z_std"]

    else:
        # Calculate mean over all scenarios
        mean_obj_value = np.mean(subproblem["sample_solutions"])
        subproblem["E"] = subproblem["Z_E"] = mean_obj_value
    ...
    if attributes["method_type"] == "KG":
        ...
        s_n = cov_df.to_numpy(dtype=float, copy=False)
        ...
        # Now, do the operation and make sure cov_df and KG_lambda and KG_mu are in the
        # right place
        ...
        # if utilities.is_pos_def(cov_matrix) != True:
        #     print("Non positive definite matrix cov_matrix")
        #     raise Exception("Non positive definite matrix cov_matrix")
        # if utilities.is_pos_semi_def(cov_matrix) != True:
        #     print("Non positive semidefinite matrix cov_matrix")
        #     raise Exception("Non positive semidefinite matrix cov_matrix")
...
def multisolve_scenarios(
    ...
    -------
    k : list
        List of solutions for each scenario.
    """
...
def bootstrap_mean_std_estimate(
    ...
    -------
    Zmean : float
        Mean, as obtained from bootstrap method.
    Zstd : float
        Std, as obtained from bootstrap method.
    """
...
def assign_scenarios(
    ...
    ----------
    partition_list : list of dict
        List of subproblems to be assigned scenarios.
    total_scenarios : list
        List of all scenarios.
    method : str
        Method to be used for assigning scenarios.

    Returns
    -------
    assigned_scenarios_dict : dict
        Dictionary with subproblem-index as key and value the assigned
    ...
    if method == "Random":
        ...
        # Assign all scenarios to a single randomly chosen subproblem of partition_list
        assigned_scenarios = np.array(
            [np.random.randint(0, len(partition_list))] * scenarios_length
        )

    elif method == "Distance":
        # Obtain random distribution of samples based on distance of its corresponding
        # (Z_E, Z_std)
...
def assign_scenarios_KG(
    ...
    # Call KG algorithm to select next index to sample
    s = cov_df.to_numpy(dtype=float)
    mu = np.fromiter(attributes["KG_mu"].values(), dtype=float)
    lambda_ = np.fromiter(attributes["KG_lambda"].values(), dtype=float)

    selected_index = kg_multi(mu, s, lambda_, attributes["pool"])

    # Convert solution to selected subproblem name
    selected_name = sorted(list(attributes["KG_mu"].keys()))[selected_index[0]]
...
def assign_scenarios_pareto(
    ...
    -------
    assigned_scenarios_dict : dict
        Dictionary with subproblem-index as key and value
...
def rank_by_distance(
    ...
    ----------
    partition_list : list of dict
        List of subproblems to be ranked by distance.

    Returns
    -------
    probabilities : list of float
        List of probabilities ranking nodes in partition_list.
    """
    distance = []
    probabilities = []
    for subproblem in partition_list:
    ...
    # Obtain probabilities based on distance
...
def pareto_fronts_probabilities(
    ...
    ----------
    partition_list : list of dict
        List of subproblems to be ranked.
    beta : float
        Parameter for Pareto method.
    probability_method : str
        Method to be used for calculating probabilities.
...
def update_row_col(
    ...
    ----------
    subproblem1 : dict
        Dictionary representing the first subproblem.
    subproblem2 : dict
        Dictionary representing the second subproblem.
    partition_list : list of dict
        List of subproblems.
    attributes : dict
        Dictionary containing attributes and parameters needed for the algorithm.
    ...
    # Update cov_df last two rows and columns
    cov_matrix = cov_df.to_numpy()
    k = cov_matrix.shape[0] - 1
    ...
    cov_matrix = cov_matrix.astype(float)
    cov_matrix = cov_nearest(cov_matrix, threshold=1e-6, n_fact=10000, return_all=False)
    s_refined_cov = cov_df.to_numpy()
    s_refined_cov[:, :] = cov_matrix

    # ! DEBUG
    # if utilities.is_pos_def(cov_matrix) is not True:
    #     print("Non positive definite matrix cov_matrix")
    #     raise Exception("Non positive definite matrix M")
    # if utilities.is_pos_semi_def(cov_matrix) is not True:
    #     print("Non positive semidefinite matrix cov_matrix")
    #     raise Exception("Non positive semidefinite matrix cov_matrix")
